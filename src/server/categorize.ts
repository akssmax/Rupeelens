...
function buildSystemPrompt(extraCategoryIds: string[] = []): string {
  const ids = [
    ...new Set([
      ...CATEGORY_IDS.filter((c) => c !== "uncategorized"),
      ...extraCategoryIds.filter((c) => c !== "uncategorized"),
    ]),
  ]
  return `You are a personal finance categorizer for Indian bank statements (UPI, IMPS, NEFT, RTGS, NACH, cards).
Given transactions, return JSON only: { "results": [ { "id", "merchant", "category", "isSubscription", "confidence" } ] }
Rules:
- category MUST be one of: ${ids.join(", ")}
...
- Wine shops, vineyards, sommelier → wine; liquor stores, bars, pubs, beer, IMFL → alcohol (not food or groceries)
...
export const categorizeTransactions = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const content = await mistralChat({
      ...
      json: true,
      messages: [
        { role: "system", content: buildSystemPrompt(data.categoryIds) },
